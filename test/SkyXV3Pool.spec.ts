...
import { TestSkyXV3SwapPay } from '../typechain/TestSkyXV3SwapPay'
...
import { TestSkyXV3ReentrantCallee } from '../typechain/TestSkyXV3ReentrantCallee'
...
describe('SkyXV3Pool', () => {
  ...
  let pool: MockTimeSkyXV3Pool

  let swapTarget: TestSkyXV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeSkyXV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  // https://github.com/skyxexchange/skyx-core-v3/issues/214
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestSkyXV3ReentrantCallee')
      ).deploy()) as TestSkyXV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestSkyXV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestSkyXV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestSkyXV3SwapPay
