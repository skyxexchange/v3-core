...
import { SkyXV3PoolSwapTest } from '../typechain/SkyXV3PoolSwapTest'
...
describe('SkyXV3Pool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('SkyXV3PoolSwapTest')
            const tester = (await testerFactory.deploy()) as SkyXV3PoolSwapTest
