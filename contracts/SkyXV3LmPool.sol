...
import './interfaces/liquidity-mining/ISkyXV3LmPoolDeveloper.sol';
import './interfaces/liquidity-mining/ILMPool.sol';

contract SkyXV3LmPool is ISkyXV3LmPool {
    ...
    ISkyXV3Pool public immutable pool;
    ...
    constructor() {
        (address poolAddress, address masterChefAddress) = ISkyXV3LmPoolDeveloper(msg.sender).parameters();
        pool = ISkyXV3Pool(poolAddress);
    ...
    function _accumulateReward(uint32 currTimestamp) internal {
        ...
        if (lmLiquidity != 0) {
            uint256 skyxPerSecond = masterChef.skyxPerSecondByPool(address(pool));
            ...
            if (duration != 0) {
                rewardGrowthGlobalX128 += FullMath.mulDiv(
                    duration,
                    skyxPerSecond * FixedPoint128.Q128,
                    lmLiquidity
                );
            }
        }
