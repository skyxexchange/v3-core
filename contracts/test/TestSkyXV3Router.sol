...
contract TestSkyXV3Router is ISkyXV3SwapCallback {
    ...
    function skyxV3SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? ISkyXV3Pool(msg.sender).token0() : ISkyXV3Pool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == ISkyXV3Pool(pools[0]).token1();
            ISkyXV3Pool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(ISkyXV3Pool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(ISkyXV3Pool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
